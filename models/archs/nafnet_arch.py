...
class AsymmetricSimpleGate(nn.Module):
    """Split input into [2C/3, C/3] = [value, gate]"""
    def forward(self, x):
        c = x.shape[1]
        gate_channel = c // 3
        val_channel = c - gate_channel # 2C / 3
        x1, x2 = torch.split(x, [val_channel, gate_channel], dim=1)
        x2 = x2.repeat_interleave(2, dim=1)
        return x1 * x2
...
class NAFBlockD(nn.Module):
    """Variant D: Asymmetric SimpleGate (2C/3 / C/3 split)"""
    def __init__(self, c, DW_Expand=3, FFN_Expand=3, drop_out_rate=0.):
        super().__init__()
        dw_channel = c * DW_Expand # 3c
        val_channel = 2 * c # gate output: 2c (vs c in baseline)
        self.conv1 = nn.Conv2d(in_channels=c, out_channels=dw_channel, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        self.conv2 = nn.Conv2d(in_channels=dw_channel, out_channels=dw_channel, kernel_size=3, padding=1, stride=1, groups=dw_channel,
                               bias=True)
        self.conv3 = nn.Conv2d(in_channels=val_channel, out_channels=c, kernel_size=1, padding=0, stride=1, groups=1, bias=True)

        self.sca = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels=val_channel, out_channels=val_channel, kernel_size=1, padding=0, stride=1,
                      groups=1, bias=True),
        )

        self.sg = AsymmetricSimpleGate()

        ffn_channel = FFN_Expand * c # 3c
        ffn_val_channel = 2 * c
        self.conv4 = nn.Conv2d(in_channels=c, out_channels=ffn_channel, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
        self.conv5 = nn.Conv2d(in_channels=ffn_val_channel, out_channels=c, kernel_size=1, padding=0, stride=1, groups=1, bias=True)
